  ...
  erst debug --network testnet --compare-network mainnet <tx-hash>`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if compareNetworkFlag == "" {
			...
			displayDetailedResults(txHash, resp, primaryResult, networkFlag, horizonURL)
			...
			go func() {
				...
				if snapshotFlag != "" {
					snap, err := snapshot.Load(snapshotFlag)
					if err != nil {
						primaryErr = fmt.Errorf("failed to load snapshot: %w", err)
						return
					}
					ledgerEntries = snap.ToMap()
				} else {
				...
				fmt.Printf("Running simulation on %s...\n", networkFlag)
				primaryReq := &simulator.SimulationRequest{
					EnvelopeXdr:   resp.ResultMetaXdr,
					ResultMetaXdr: resp.ResultMetaXdr,
					LedgerEntries: ledgerEntries,
				}
			...
			go func() {
				...
				// Fetch entries from compare network
				var compareEntries map[string]string
				compareResp, err := compareClient.GetTransaction(ctx, txHash)
				if err != nil {
					compareErr = fmt.Errorf("failed to fetch transaction from %s: %w", compareNetworkFlag, err)
					return
				}

				compareEntries, err = rpc.ExtractLedgerEntriesFromMeta(compareResp.ResultMetaXdr)
				if err != nil {
					logger.Logger.Warn("Failed to extract ledger entries from compare network", "error", err)
					compareEntries = nil
				}
...
func displayDetailedResults(txHash string, txResp *rpc.TransactionResponse, simResp *simulator.SimulationResponse, network, horizonURL string) {
